...
1. Run the command `npm install` or `yarn install` to install the dependencies
2. After, run the command ` yarn or npm run init-db` to install the database
3. And to start the application, run the command `yarn or npm start` 
...
Made with ♥ by David com Amor e ajuda da Rocketseat
